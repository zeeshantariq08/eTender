    ...
    <form method="POST" class="form-horizontal form-bordered" action="{{ ($usergroup->slug) ? route('usergroups.update', $usergroup->slug ) : route('usergroups.store') }}">
        ...
        <div class="form-group @error('name') has-error @enderror @if(session('danger')) has-error @endif">
            ...
            <div class="col-md-9">
                ...
                <select class="form-control z-index show-tick" name="permissions[]" data-live-search="true"  multiple required>
                    <option value="">Select Permissions</option>
                    @foreach($permissions as $id => $permissions)
                        <option value="{{ $id }}" {{ (in_array($id, old('permissions', [])) || $usergroup->permissions->contains($id)) ? 'selected' : '' }}>{{ $permissions }}</option>
                    @endforeach
                </select>
