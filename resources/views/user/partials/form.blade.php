...
@section('b-content')
 ...
 --}}
    ...
    <form method="POST" class="form-horizontal form-bordered" action="{{ ($user->id) ? route('users.update', $user->id ) : route('users.store') }}">
                ...
                <div class="col-md-9">
                    <select id="user_group_id" name="user_group_id" class="form-control" size="1" required>
                        ...
                        @foreach( $usergroups as $usergroup)
                            <option @if($user->user_group_id === $usergroup->id) selected @endif value="{{$usergroup->id}}">{{ $usergroup->name }}</option>
                        @endforeach
